...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for i in self.all_actions:
            temp_node = PgNode_a(i)
            precond = temp_node.prenodes
            if precond.issubset(self.s_levels[level]):
                self.a_levels[level].add(temp_node)
                for j in self.s_levels[level]:
                    if {j}.issubset(precond):
                        j.children.add(temp_node)
                        temp_node.parents.add(j)
    ...
    def add_literal_level(self, level):
        ...
        for i in self.a_levels[level-1]:
            child = i.effnodes
            # temp_node = PgNode_s(child, True)
            for j in child:
                self.s_levels[level].add(j)
                j.parents.add(i)
                i.children.add(j)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        temp1 = node_a1.effnodes
        temp2 = node_a2.effnodes
        count = 0
        total = 0
        for i in temp1:
            total = total + 1
            for j in temp2:
                if i.symbol == j.symbol and i.is_pos != j.is_pos:
                    count = count+1
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        if count == total and total!=0:
            return True
        temp3 = node_a1.effnodes
        temp4 = node_a2.prenodes
        count = 0
        total = 0
        for i in temp3:
            total = total + 1
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        temp1 = node_a1.parents
        temp2 = node_a2.parents
        for i in temp1:
            for j in temp2:
                if i.is_mutex(j):
                    return True
        temp1 = node_a2.parents
        temp2 = node_a1.parents
        for i in temp1:
            for j in temp2:
                if i.is_mutex(j):
                    return  True
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        temp1 = node_s1
        temp2 = node_s2
        if temp1.symbol == temp2.symbol and temp2.is_pos != temp1.is_pos:
            return True
        return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        '''
        action_nodes_1 = node_s1.parents
        action_nodes_2 = node_s2.parents
        poss = True
        for node_1 in action_nodes_1:
            for node_2 in action_nodes_2:
                if node_1.is_mutex(node_2) and node_2!= node_1:
                    garb = 0
                else:
                    poss = False
        if poss:
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in goal_list:
            index = 0
            found = 0
            for states_list in self.s_levels:
                for state in states_list:
                    if state.symbol == goal and state.is_pos == True:
