...
class CornersProblem(search.SearchProblem):
  ...
  def isGoalState(self, state):
    ...
    # no food state corresponds to empty tuple
    if len(state[1]) !=0:
        isGoal = False
...
def cornersHeuristic(state, problem):
  ...
  #calculate nearest possible food
...
def foodHeuristic(state, problem):
    ...
    # basic state , no food no solution :)
    if len(food) == 0:
        return 0

    # utilizing the maze distance function is beneficial since it expands
    # only 4k nodes but takes 40 s time for the problem , quite longer than rest
    # this is consistent since it solves problem of finding distance ot farthest
    # piece of food and will always find same distance to given food
    # it will also return 0 at a goal , nearest distance  at that point being
    # the one at which pacman is and will never be -ve
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        # performing a bfs to search nearest food
        fringe = util.Queue()
        visited = []        # visited nodes
        action_list = []    # actions to reach current position
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        """
        # required goal state is if paccy is on a current tile/node with food
        x,y = state
...
class ApproximateSearchAgent(Agent):
            ...
            for i in range(currx-2, currx+1):
                for j in range(curry-2, curry+1):
                    if i >=0 and j>= 0 and i <= self.right and j <= self.top and foodGrid[i][j] and not walls[i][j]:
                        score = util.manhattanDistance(state.getPacmanPosition(), (i, j)), 0, (i, j)
                        temp.append(score)
            if not temp:
                for x, row in enumerate(foodGrid):
                    for y, cell in enumerate(row):
    ...
    def foodHeuristic(self, state, problem):
        ...
        # heuristic similar to above implemented food heuristic , farthest food
        total = []
        for x, row in enumerate(foodGrid):
            for y, cell in enumerate(row):
                if foodGrid[x][y]:
